...
from maythusharmusic import cailin, app, db, lang, queue, tg, yt
from maythusharmusic.helpers import admin_check, buttons, can_manage_vc
...
async def _controls(_, query: types.CallbackQuery):
    ...
    if action == "pause":
        ...
        await cailin.play_next(chat_id)
        ...
        return await cailin.play_media(chat_id, msg, media)

    elif action == "replay":
        media = queue.get_current(chat_id)
        media.user = user
        await cailin.replay(chat_id)
        status = query.lang["replayed"]
        reply = query.lang["play_replayed"].format(user)

    elif action == "stop":
        await cailin.stop(chat_id)
